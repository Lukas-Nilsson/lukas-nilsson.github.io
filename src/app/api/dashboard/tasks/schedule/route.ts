...
import { createTimeEntry, updateTimeEntry, deleteTimeEntry, getAllTasks } from '@/lib/clickup';
 ...
 * 
 * Body: { entry_id, start_time? (ISO), end_time? (ISO) }
 */
export async function PATCH(req: NextRequest) {
    ...
    try {
        const { entry_id, start_time, end_time } = await req.json();
        if (!entry_id) {
        ...
        const entry = await updateTimeEntry(entry_id, updates);
